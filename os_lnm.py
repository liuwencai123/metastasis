...
#应用标题
st.title('A machine learning-based predictive model for predicting lymph node metastasis in patients with osteosarcoma')
...
map = {'Left':0,'Right':1,'Other':2,'T1':0,'T2':1,'T3':2,'TX':3,'M0':0,'M1':1,'No':0,'Yes':1,}
#Age =map[Age]
...
thyroid_train['LNM'] = thyroid_train['LNM'].apply(lambda x : +1 if x==1 else 0)
#thyroid_test = pd.read_csv('test.csv', low_memory=False)
#thyroid_test['BM'] = thyroid_test['BM'].apply(lambda x : +1 if x==1 else 0)
features = ['Age','Laterality','T','M','surgery','Radiation','Chemotherapy','Bone_metastases','Lung_metastases']
target = 'LNM'
...
sp = 0.5
#figure
is_t = (XGB.predict_proba(np.array([[Age,Laterality,T,M,surgery,Radiation,Chemotherapy,Bone_metastases,Lung_metastases]]))[0][1])> sp
prob = (XGB.predict_proba(np.array([[Age,Laterality,T,M,surgery,Radiation,Chemotherapy,Bone_metastases,Lung_metastases]]))[0][1])*1000//1/10
